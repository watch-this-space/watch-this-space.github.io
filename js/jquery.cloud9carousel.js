...
;(function($) {
  ...
  var Carousel = function( element, options ) {
    ...
    this.renderItem = function( itemIndex, rotation ) {
      ...
      item.moveTo(
        this.xOrigin + (scale * ((Math.cos(rotation) * this.xRadius*(this.container).width()) - (item.fullWidth * 0.5))),
    ...
    // own addition: send to specific card
    this.sendToIndex = function( idx ) {
      if( idx >= 0 ){
    ...
    this.bindControls = function() {
      ...
      if( options.bringToFront ) {
        $container.bind( 'click.cloud9', function( event ) {
          ...
          if( options.menuMove ) {
            var hits2 = $(event.target).closest( '.' + options.menuClass + ' li');
            // oldIdx = self.items.indexOf(self.nearestItem())
            if( hits2.length !== 0 ) {
              idx = self.menuItems.indexOf( hits2[0] );
              
              self.sendToIndex( idx );
            }
          }
    ...
    var menuItems = $container.find( '.' + options.menuClass + ' li' );
    ...
    this.finishInit = function() {
      ...
      this.bindControls();
      //this.render();
      this.renderTimer = setInterval( function() { self.render() }, 50 );
      this.scaleTimer = setInterval( function() { self.scale() }, 50 );
